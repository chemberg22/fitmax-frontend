...
async function fetchAppointments() {
    ...
    // Busca a data via URL
    const selectedDate = getDateFromUrl();
    // Verifica se a data é válida, se não, loga e alerta o erro redirecionando para a tela anterior
    if (!selectedDate) {
    ...
    try {
        ...
        // Obter valor do filtro de modalidade
        const modalidadeFilter = document.getElementById('modalidade-filter').value;

        // Monta e loga a URL da requisição a API de agendamentos com os parâmetros aplicados
        let url = `http://localhost:8080/api/agendamentos?dataInicio=${selectedDate}&dataFim=${selectedDate}&personalId=${personalId}`;
